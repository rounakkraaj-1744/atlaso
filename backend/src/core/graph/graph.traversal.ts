...
export function bfs( graph: Graph, startNodeId: string, direction: 'forward' | 'backward' = 'forward' ): string[] {
    ...
    while (queue.length > 0) {
        ...
        const neighbors = direction === 'forward' ? getOutgoing(graph, nodeId) : getIncoming(graph, nodeId);
...
export function dfs( graph: Graph, startNodeId: string, direction: 'forward' | 'backward' = 'forward' ): string[] {
    const visited = new Set<string>();
    const result: string[] = [];
...
export function findAllPaths( graph: Graph, startId: string, endId: string, maxDepth = 10 ): string[][] {
    const paths: string[][] = [];
...
export function topologicalSort(graph: Graph): string[] | null {
    ...
    // starting with nodes have 0-in-degress
